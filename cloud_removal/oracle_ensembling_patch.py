...
if(use_VPint=="True"):
    ensemble_name = "PatchOracleEnsembleVPint"
    methods = ["VPint","replacement","regression","ARMA"]
    
else:
    ensemble_name = "PatchOracleEnsembleNoVPint"
...
for patchname in os.listdir(currdir):
    ...
    for feature in features:
        ...
        if(replace or not(os.path.exists(fn))):
            ...
            # Compute MAEs of pred grids
            maes = np.zeros(len(pred_grids))
            for i in range(0,len(pred_grids)):
                maes[i] = np.nanmean(np.absolute(pred_grids[i]-target_grid))
                
            best_ind = np.argmin(maes)
            ensemble_pred_grid = pred_grids[best_ind]
