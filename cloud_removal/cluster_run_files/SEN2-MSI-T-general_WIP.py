...
def run_patch(base_path, legend, scene, y_size, x_size, y_offset, x_offset, feature_name="img_1m", plot=False,
    ...
    if(algorithm=="VPint"):
        ...
        pred_grid = baselines.NSPI.NSPI(target_cloudy, features[:,:,:,0], mask)
    elif(algorithm == "WLR"):
        pred_grid = baselines.WLR.WLR(target_cloudy, features[:,:,:,0], mask)
    else:
        pred_grid = None
...
if(len(sys.argv) != 5):
    print("Usage: python SEN2-MSI-T-general.py [dataset base path] [result base path] [algorithm set] [task number]")
...
#conditions_scenes = ["america_cropland_iowa"]
...
for y_offset in row_list:
    for x_offset in col_list:
        ...
        if(replace or not(os.path.exists(path))):
            #try:
            pred_grid = run_patch(base_path, legend, this_run_cond["scene"], size_y, size_x, y_offset, x_offset, feature_name=this_run_cond["features"], plot=False, algorithm=this_run_cond["algorithm"], buffer_mask=False)
